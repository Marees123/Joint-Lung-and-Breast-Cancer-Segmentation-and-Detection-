...
# Improved Chaotic Puma Optimizer Algorithm (ICPOA) ->

def Proposed(X, objective_function, VRmin, VRmax, max_iter):
    ...
    for t in range(max_iter):
        ...
        for i in range(num_pelicans):
            ...
            r1 = (np.mean(fitness) - min(fitness)) / (fitness[i] + np.mean(fitness) + max(fitness))
            r2 = (np.mean(fitness) - min(fitness)) / (fitness[i] + np.mean(fitness) + max(fitness))
